 ...
 */
@Deprecated
public class OktaCliUserAgentProvider implements UserAgentProvider {

    private static final Logger LOG = LoggerFactory.getLogger(OktaCliUserAgentProvider.class);
    ...
    private static Map<String, String> oktaComponentsFromVersionMetadata() {
        ...
        try {
            list(ApplicationInfo.class.getClassLoader().getResources(VERSION_FILE_LOCATION)).stream()
                    .map(OktaCliUserAgentProvider::loadProps)
